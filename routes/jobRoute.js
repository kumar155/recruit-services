...
router.get("/jobscalendar/:id", async function (req, res, next) {
    try {
        res.json(await jobService.getJobsCalendar(req.params.id));
    } catch (err) {
        console.error(`Error while getting the jobs calendar `, err.message);
        next(err);
    }
});
